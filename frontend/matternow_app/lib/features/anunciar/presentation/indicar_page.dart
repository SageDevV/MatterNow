...
/// Tela "Dica de mãe" — replica o frame de compartilhamento de indicações.
/// Estrutura espelha [PerguntarPage] reaproveitando os mesmos temas, modais
/// de faixa etária/origem de foto e galeria. Diferenças do Figma:
/// - Cabeçalho "Dica de mãe"
/// - Heading "Escolha o tema da sua dica"
/// - Label do campo principal "Qual a sua dica"
/// - CTA "Publicar a dica de mãe 💛"
class IndicarPage extends StatefulWidget {
  const IndicarPage({super.key, required this.repository});
  final AnuncioRepository repository;

  @override
  State<IndicarPage> createState() => _IndicarPageState();
}

class _IndicarPageState extends State<IndicarPage> {
  static const _alturaCabecalho = 60.0;
  static const _maxFotos = 8;
  static const _maxDica = 100;

  final _formKey = GlobalKey<FormState>();
  final _dicaCtrl = TextEditingController();
  ...
  void initState() {
    super.initState();
    _dicaCtrl.addListener(() => setState(() {}));
  }
  ...
  Future<void> _abrirFaixaEtaria() async {
    final escolhida =
        await FaixaEtariaModal.mostrar(context, selecionada: _faixaEtaria);
  ...
  Future<void> _publicar() async {
    ...
    if (_tema == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Escolha o tema da sua dica.')),
      );
      return;
    }
    ...
    try {
      await widget.repository.compartilharDica(
        NovaDica(
          tema: _tema!,
          dica: _dicaCtrl.text.trim(),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Dica publicada! 💛')),
      );
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _LabelComContador(
                      label: 'Qual a sua dica',
                      atual: _dicaCtrl.text.length,
                      maximo: _maxDica,
                    ),
                    const SizedBox(height: 8),
                    _CampoTexto(
                      controller: _dicaCtrl,
                      hint: 'Cadeira de amamentação super confortável',
                      maxLength: _maxDica,
                      validator: (v) =>
                          (v ?? '').trim().isEmpty ? 'Escreva sua dica.' : null,
                    ),
                    const SizedBox(height: 16),
                    ...
                    _CampoTexto(
                      ...
                      hint:
                          'Mães, comprei essa cadeira ali no centro de Floripa, barata e super a vale a pena',
                      maxLines: 4,
                    ),
                    const SizedBox(height: 16),
...
class _ChipTema extends StatelessWidget {
  const _ChipTema(
      {required this.tema, required this.ativo, required this.onTap});
...
class _DottedSquare extends StatelessWidget {
  const _DottedSquare({required this.child});
  final Widget child;
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 4),
                Text(
                  'Compartilhe com carinho. Dicas que desrespeitam nossas regras podem ser removidas.',
                  style: TextStyle(
                    color: Color(0xFF6B7280),
                    fontSize: 12,
                    height: 1.4,
